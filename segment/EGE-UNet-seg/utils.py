...
def save_imgs(img, msk, msk_pred, i, save_path, datasets, threshold=0.5, test_data_name=None):
    ...
    img_extracted=np.zeros((256,256,4))
    img_extracted[:,:,0:3]=img
    img_extracted[:,:,3]=msk_pred
...
def save_imgs_all(img, msk_pred, save_path, datasets, threshold=0.5, test_data_name=None):
    img = img.squeeze(0).permute(1,2,0).detach().cpu().numpy()
    img = img / 255. if img.max() > 1.1 else img
    ...
    plt.imshow(img_extracted)
    plt.axis('off')
    plt.savefig(save_path + test_data_name)
    plt.close()
